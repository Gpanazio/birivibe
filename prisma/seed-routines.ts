...
async function main() {
  ...
  const habitByName = (name: string) => habits.find(h => h.name.toLowerCase().includes(name.toLowerCase()));
    ...
    data: {
      ...
      steps: {
        create: [
          ...
          { name: "Beber água", icon: "💧", duration: 2, type: "habit", order: 1, isOptional: false, habitId: habitByName("água")?.id },
          { name: "Tomar remédios", icon: "💊", duration: 2, type: "habit", order: 2, isOptional: false, habitId: habitByName("remédio")?.id },
  ...
  console.log(`✓ Criada: ${rotinaManha.name} (${rotinaManha.steps ? 5 : 0} passos)`);
